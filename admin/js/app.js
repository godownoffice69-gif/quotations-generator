...
const OMS = {
    ...
    festivals: {
        "2025-01-14": ["मकर संक्रांति", "Makar Sankranti"],
        "2025-01-26": ["गणतंत्र दिवस", "Republic Day"],
        "2025-02-13": ["महा शिवरात्रि", "Maha Shivratri"],
        "2025-03-13": ["होली", "Holi"],
        "2025-03-14": ["धुलेटी", "Dhuleti"],
        "2025-04-06": ["राम नवमी", "Ram Navami"],
        "2025-08-15": ["स्वतंत्रता दिवस", "Independence Day"],
        "2025-08-19": ["जन्माष्टमी", "Janmashtami"],
        "2025-10-02": ["गांधी जयंती", "Gandhi Jayanti"],
        "2025-11-01": ["धनतेरस", "Dhanteras"],
        "2025-11-02": ["दीवाली", "Diwali"]
    },
...
const functionHTML = `
    <div class="card" style="margin-bottom: 1rem;" id="${functionId}Card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
            ...
            ${!isFirst ? `<button type="button" class="btn btn-danger btn-small" onclick="OMS.removeFunctionFromDay(${dayIndex}, ${functionIndex})">🗑️ Remove</button>` : ''}
        </div>
        ...
        <tbody>
            ${items.map((item, idx) => `
                <tr>
                    ...
                    <td>
                        <button class="btn btn-danger btn-small" onclick="OMS.removeFunctionItem(${dayIndex}, ${functionIndex}, ${idx})">🗑️</button>
                    </td>
                </tr>
            `).join('')}
        </tbody>
        ...
        <tbody>
            ${items.map((item, idx) => `
                <tr>
                    ...
                    <td>
                        <button class="btn btn-danger btn-small" onclick="OMS.removeDayItem(${dayIndex}, ${idx})">🗑️</button>
                    </td>
                </tr>
            `).join('')}
        </tbody>
...
this.updateDayItemsList(dayIndex);
    ...
    setupAutoBackup() {
        ...
        console.log('✅ Auto-backup system initialized (exports every 24h or 50 changes)');
    },
    ...
    autoExportData() {
        try {
            ...
            this.showToast('📦 Auto-backup created', 'success');
        } catch (error) {
    ...
    manualExport() {
        this.autoExportData();
        this.showToast('📥 Manual backup created!', 'success');
    },
...
const snapshot = await db.collection('orders')
    .where('orderId', '==', id)
    .get();
    console.log('🔥 Deleting orderId:', id);
...
await batch.commit();
console.log(`🔥 Deleted ${snapshot.size} Firestore doc(s) for orderId ${id}`);
...
order: async () => {
    const index = this.data.orders.findIndex(o => o.orderId === id);
    if (index === -1) {
console.warn('⚠️ Order not found in local array:', id);
return false;
    }

    try {
console.log('🔄 Attempting to delete orderId:', id);
...
if (docSnap.exists) {
    await docRef.delete();
    console.log('✅ Deleted Firestore doc with ID:', id);
} else {
    console.warn('⚠️ No Firestore document found with ID:', id);
...
this.saveToStorage();
console.log(`📝 Saved deletedOrders list (${this.data.deletedOrders.length} total)`);
...
if (manualOrderId && manualOrderId.startsWith('FP')) {
    ...
    this.showToast('⚠️ Completed orders require a manual FP ID (e.g., FP001)', 'error');
    return;
}
    } else {
...
console.log('✏️ Editing existing order by docId:', this.editingDocId);
    } else {
existingSnapshot = await db.collection('orders')
    .where('orderId', '==', this.editingOrderId)
    .get();
console.log('✏️ Editing existing order:', this.editingOrderId);
    }
...
this.showToast(`Order upgraded: ${oldOrderId || '[No ID]'} → ${finalOrderId} ✅`);
    } else {
...
this.showToast('Order updated! ✏️');
    }
    ...
    if (finalOrderId) {
        ...
        console.log('📝 Auto-generated docId:', docRef.id);
    }
...
if (!localExists) {
    this.data.orders.push(completeOrderData);
        ...
        this.showToast(`Order ${displayId} saved to Firestore! 🆕`);
        this.refreshOrderId();
        this.saveToStorage();
        this.updateAllDisplays();
...
if (inventoryItem) {
    ...
    if (inventoryItem.quantity <= this.data.settings.lowStockThreshold) {
        this.showToast(`⚠️ LOW STOCK: ${inventoryItem.name} (${inventoryItem.quantity} left)`, 'warning');
    }

    // Negative stock alert
    if (inventoryItem.quantity < 0) {
        this.showToast(`🚨 CRITICAL: ${inventoryItem.name} stock is NEGATIVE!`, 'error');
    }
}
    });
...
this.data.orderAuditLog[identifier] = [];
    ...
    console.log(`📝 Audit log: ${entry.action} by ${entry.user}`);
},
...
setTimeout(() => {
    ...
    duplicateOrder(identifier) {
        ...
        if (confirm(`Duplicate order for "${order.clientName}"?\n\nThis will create a new order with the same details.`)) {
            ...
            this.switchTab('orders');
            this.showToast(`✅ Order duplicated! Review and save.`, 'success');
    ...
    buildOrderHTML(order, fontSize, colors, compact = false) {
        console.log('🖼️ Building image for order:', order.orderId);
        console.log('🖼️ isMultiDay:', order.isMultiDay);
        console.log('🖼️ dayWiseData:', order.dayWiseData);
        ...
        return `
            <div style="margin-bottom: ${compact ? '40px' : '60px'}; ${compact ? 'border: 2px solid ' + colors.tableBorder + '; padding: 20px;' : ''}; color: #000000;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; font-size: ${fontSize}px; margin-bottom: 20px; color: #000000;">
                    <div>
                        ${order.isMultiDay ? `
                            <strong style="color: #000000;">Start Date:</strong> ${Utils.formatDate(order.startDate) || 'N/A'}<br>
                            <strong style="color: #000000;">End Date:</strong> ${Utils.formatDate(order.endDate) || 'N/A'}<br>
                        ` : `
                            <strong style="color: #000000;">Date:</strong> ${Utils.formatDate(order.date) || 'N/A'}<br>
                            <strong style="color: #000000;">Time:</strong> ${order.readyTime || '-'}<br>
                        `}
                    ...
                    <div>
                        ...
                        <strong style="color: #000000;">Status:</strong> ${order.status || '-'}
                    </div>
                </div>
                                    ...
                                    day.functions.map((func, funcIdx) =>
                                        '<div style="background: white; padding: 15px; border: 1px solid ' + colors.tableBorder + '; border-radius: 6px; margin-top: ' + (funcIdx > 0 ? '15px' : '0') + '; color: #000000;">' +
                                            ...
                                            '<div style="font-size: ' + fontSize + 'px; margin-bottom: 10px; color: #000000;">' +
                                                ...
                                                '<strong style="color: #000000;">Time:</strong> ' + (func.timeSlot || '-') + '<br>' +
        ...
        let html = `
            <div class="calendar-container">
                <div class="calendar-header">
                    <div class="calendar-nav">
                        <button class="btn btn-primary btn-small" onclick="OMS.changeMonth(-1)">←</button>
            ...
            <div class="card">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <button class="btn btn-primary" onclick="OMS.addTask()">➕ Add</button>
                    </div>
                </div>
            </div>
        `;
    ...
    showDayDetails(dateStr) {
        ...
        if (orders.length > 0) {
            content += `<h4>📋 Orders (${orders.length})</h4>`;
            orders.forEach(o => {
                const color = this.getOrderColor(o);
                let dateInfo = '';
                if (o.isMultiDay) {
                    dateInfo = `<br><small style="color:#666;">📅 Multi-day: ${Utils.formatDate(o.startDate)} to ${Utils.formatDate(o.endDate)}</small>`;
                }
                content += `
        ...
        if (tasks.length > 0) {
            content += `<h4>📝 Tasks (${tasks.length})</h4><ul>`;
            tasks.forEach(t => {
                content += `<li>
                    ${t.description}
    ...
    renderDashboard() {
        ...
        this.renderTable('recentOrdersContainer', [
            ...
            <button class="btn btn-info btn-small" onclick="OMS.duplicateOrder('${row.orderId || row.docId}')">📋 Duplicate</button>
    ...
    renderInventory() {
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <div class="btn-group">
                    <button class="btn btn-warning" onclick="OMS.batchUpdateInventory()">⚡ Batch Update</button>
                </div>
                <div class="card">
                    <h3>Categories</h3>
                    <div class="form-row">
    ...
    updateInventoryDisplay() {
        ...
        this.renderTable('itemsContainer', [
            ...
            <button class="btn btn-primary btn-small" onclick="OMS.editInventoryItem('${row.id}')">✏️ Edit</button>
            <button class="btn btn-danger btn-small" data-action="delete" data-type="item" data-id="${row.id}">🗑️ Delete</button>
        `);
    },
    ...
    editInventoryItem(id) {
        ...
        this.showModal('Edit Inventory Item', `
            ...
            <div class="btn-group">
                <button class="btn btn-primary" onclick="OMS.saveInventoryItemEdit('${id}')">💾 Save Changes</button>
                <button class="btn btn-secondary" onclick="OMS.closeModal()">Cancel</button>
            </div>
        `);
    },
    ...
    renderPreparation() {
        ...
        container.innerHTML = `
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title">📋 Preparation & Forecast</h2>
    ...
    renderForecastResults(orders, items, startDate, endDate) {
        ...
        container.innerHTML = `
            ...
            <div class="card" style="background: linear-gradient(135deg, var(--primary), var(--secondary)); color: white; padding: 1.5rem;">
                <h3 style="margin-bottom: 1rem; font-size: 1.3rem;">📊 Summary for ${Utils.formatDate(startDate)} to ${Utils.formatDate(endDate)}</h3>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem;">
                    ...
                    <div style="background: rgba(255,255,255,0.2); padding: 1rem; border-radius: var(--radius); text-align: center;">
                        ...
                        <div style="font-size: 0.9rem; opacity: 0.9;">⚠️ Shortages</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.2); padding: 1rem; border-radius: var(--radius); text-align: center;">
                        <div style="font-size: 2rem; font-weight: 700; color: #86efac;">${okItems.length}</div>
                        <div style="font-size: 0.9rem; opacity: 0.9;">✅ Sufficient</div>
                    </div>
                </div>
            </div>
            ...
            <!-- Export Buttons -->
            <div class="btn-group">
                <button class="btn btn-success" onclick="OMS.printForecast()">🖨️ Print</button>
                <button class="btn btn-info" onclick="OMS.exportForecastPDF()">📄 Export PDF</button>
                <button class="btn btn-warning" onclick="OMS.shareForecastWhatsApp()">📱 Share WhatsApp</button>
            </div>

            <!-- Items Table -->
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">📦 Items Preparation List</h3>
                </div>
                        ...
                        <tbody>
                            ${items.map((item, index) => {
                                const statusBadge = item.difference >= 0
                                    ? `<span style="background: var(--success); color: white; padding: 0.25rem 0.75rem; border-radius: 12px; font-size: 0.85rem;">✅ OK</span>`
                                    : `<span style="background: var(--danger); color: white; padding: 0.25rem 0.75rem; border-radius: 12px; font-size: 0.85rem;">⚠️ SHORT ${item.difference}</span>`;
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">📋 Order Details Breakdown</h3>
                </div>
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.75rem;">
                                    <div>
                                        ...
                                        <div style="font-size: 0.9rem; color: var(--text-gray);">
                                            📍 ${order.venue} |
                                            📅 ${order.isMultiDay ? `${Utils.formatDate(order.startDate)} to ${Utils.formatDate(order.endDate)}` : Utils.formatDate(order.date)} |
                                            🕐 ${order.readyTime || 'N/A'}
                                        </div>
                                    </div>
    ...
    showItemOrderDetails(itemName) {
        ...
        this.showModal(`
            <h2 style="margin-bottom: 1rem;">📦 ${itemName}</h2>
    ...
    printForecast() {
        ...
        printWindow.document.write(`
            ...
            <body>
                <h1>📋 Preparation & Forecast Report</h1>
                    ...
                    <tbody>
                        ${items.map((item, index) => `
                            <tr class="${item.difference < 0 ? 'shortage' : ''}">
                                ...
                                <td>${item.difference >= 0 ? '✅ OK' : '⚠️ SHORT'}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
    ...
    shareForecastWhatsApp() {
        ...
        let message = `📋 *PREPARATION FORECAST*\n\n`;
        message += `📅 *Period:* ${Utils.formatDate(startDate)} to ${Utils.formatDate(endDate)}\n\n`;
        message += `📊 *SUMMARY*\n`;
        message += `• Total Orders: ${stats.totalOrders}\n`;
        message += `• Confirmed: ${stats.confirmed}\n`;
        message += `• Total Items: ${items.length}\n`;
        message += `• Total Quantity: ${stats.totalItems}\n`;
        message += `• Shortages: ${stats.shortages}\n\n`;

        message += `📦 *ITEMS NEEDED*\n`;
        message += `━━━━━━━━━━━━━━━━━\n`;

        items.forEach((item, index) => {
            const status = item.difference >= 0 ? '✅' : '⚠️';
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <div class="form-row">
                    ...
                    <button class="btn btn-primary" onclick="OMS.refreshCustomerDatabase()">🔄 Refresh</button>
                </div>
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <div class="card">
                    ...
                    <div class="btn-group">
                        <button class="btn btn-primary" onclick="OMS.addTeamMember()">➕ Add Team Member</button>
                        <button class="btn btn-secondary" onclick="OMS.clearTeamForm()">🔄 Clear Form</button>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="form-row" style="margin-bottom: 1rem;">
                        <select id="teamRoleFilter" class="form-select" onchange="OMS.renderTeamTable()">
                            ...
                            <option value="Operator">⚙️ Operators</option>
    ...
    populateTeamDropdowns() {
        ...
        if (helperSelect) {
            ...
            helperSelect.innerHTML = '<option value="">Select Helper</option>' +
                helpers.map(h => `<option value="${h.name}" data-id="${h.id}">🤝 ${h.name}</option>`).join('');
        }
    },
    ...
    populateFunctionTeamDropdowns(dayIndex, functionIndex) {
        ...
        if (driverSelect) {
            ...
            driverSelect.innerHTML = '<option value="">Select Driver</option>' +
                drivers.map(d => `<option value="${d.name}" data-id="${d.id}" ${d.name === currentValue ? 'selected' : ''}>🚗 ${d.name}</option>`).join('');
        }
        ...
        if (helperSelect) {
            ...
            helperSelect.innerHTML = '<option value="">Select Helper</option>' +
                helpers.map(h => `<option value="${h.name}" data-id="${h.id}" ${h.name === currentValue ? 'selected' : ''}>🤝 ${h.name}</option>`).join('');
        }
    },
    ...
    sendTeamMemberNotification(teamMember, role, orderDetails) {
        ...
        const itemsList = items && items.length > 0
            ? items.map(item => `• ${item.name}: ${item.quantity}${item.remarks ? ' (' + item.remarks + ')' : ''}`).join('\n')
            : '• No items added yet';
        ...
        // Create WhatsApp message
        let message = `🎉 *NEW ORDER ASSIGNED TO YOU*\n\n`;
        message += `📋 *Order ID:* ${orderId}\n`;
        message += `👤 *Client:* ${clientName}\n`;
        message += `📍 *Venue:* ${venue}\n`;
        message += `📅 *Date:* ${Utils.formatDate(date)}\n`;
        message += `🕐 *Ready Time:* ${readyTime || 'TBD'}\n\n`;
        message += `🎭 *Your Role:* ${role.toUpperCase()}\n\n`;
        ...
        if (contact) {
            message += `📞 *Client Contact:* ${contact}\n\n`;
        }

        message += `⚡ Please confirm receipt by replying "OK"\n\n`;
    ...
    sendManagerReport() {
        ...
        let message = `📊 *TEAM ALLOCATION REPORT*\n`;
        message += `📅 *Date:* ${new Date().toLocaleDateString()}\n`;
        message += `📋 *Period:* Next 7 Days\n\n`;
        ...
        Object.keys(ordersByDate).sort().forEach(date => {
            message += `📅 *${Utils.formatDate(date)}*\n\n`;

            ordersByDate[date].forEach((order, index) => {
                message += `*ORDER #${index + 1}: ${order.orderId}*\n`;
                message += `👤 Client: ${order.clientName}\n`;
                message += `📍 Venue: ${order.venue}\n`;
                message += `🕐 ${order.readyTime || 'TBD'}\n`;
                message += `📊 Status: ${order.status || 'Pending'}\n`;

                if (order.driverName || order.operator || order.helper) {
                    message += `👷 Team:\n`;
                    if (order.driverName) message += `  • Driver: ${order.driverName}\n`;
                    if (order.operator) message += `  • Operator: ${order.operator}\n`;
                    if (order.helper) message += `  • Helper: ${order.helper}\n`;
                } else {
                    message += `⚠️ *Not Allocated Yet*\n`;
                }
                message += `\n`;
            });
        });
    ...
    renderHistory() {
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <div class="btn-group">
                    <button class="btn btn-success" data-action="exportData">📥 Export</button>
                </div>
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title">📈 Business Analytics</h2>
    </div>
...
<div class="card">
    <h3 class="card-title">📍 Visitor Locations</h3>
    <div class="stats-grid">
    ...
    renderSettings() {
        ...
        container.innerHTML = `
            <div class="card">
                ...
                <div class="btn-group">
                    <button class="btn btn-primary" onclick="OMS.saveSettings()">💾 Save All Settings</button>
                    <button class="btn btn-secondary" onclick="OMS.resetSettings()">🔄 Reset to Defaults</button>
                </div>

                <div class="card">
                    <h3>📊 Data Management</h3>
                    <div class="btn-group">
                        <button class="btn btn-info" data-action="exportData">📤 Export JSON</button>
                        <button class="btn btn-success" onclick="OMS.exportData('csv')">📊 Export CSV</button>
                        <button class="btn btn-warning" data-action="importData">📥 Import Backup</button>
                        <button class="btn btn-danger" onclick="OMS.resetAllData()">🗑️ Reset All Data</button>
                    </div>
                </div>
            </div>
        `;
                ...
                <tbody>
                    ${this.currentOrderItems.map((item, i) => `
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-danger btn-small" onclick="OMS.removeOrderItem(${i})">🗑️</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
...
ordersSnapshot.forEach(doc => {
    ...
    if (wasDeleted) {
        console.warn(`⚠️ Skipping deleted order: ${orderId || docId} (found in deletedOrders list)`);
        skippedDeleted++;
        return;
    }
    ...
    if (orderData.isMultiDay) {
        console.log(`📅 Loaded multi-day order ${orderId}:`, {
...
console.log(`✅ Loaded ${this.data.orders.length} orders from Firestore (skipped ${skippedDeleted} deleted)`);
...
async loadTrackingFromFirestore() {
    try {
console.log('📊 Loading tracking data from Firestore...');
...
console.log(`✅ Loaded ${this.data.trackingData.length} tracking events`);
...
console.log(`✅ Loaded ${this.data.team.length} team members from Firestore`);
return true;
    } catch (error) {
...
console.log('✅ Inventory category saved to Firestore');
return true;
    } catch (error) {
...
console.log('✅ Inventory item saved to Firestore');
return true;
    } catch (error) {
...
console.log(`📦 Firestore inventory: ${categories.length} categories, ${items.length} items (TOTAL: ${firestoreTotalCount})`);
console.log(`📦 Current local inventory: ${currentCatCount} categories, ${currentItemCount} items (TOTAL: ${currentTotalCount})`);
...
if (firestoreTotalCount > currentTotalCount) {
    ...
    console.log(`✅ Firestore has MORE data (${firestoreTotalCount} vs ${currentTotalCount}) - loading from Firestore`);
    this.data.inventory = { categories, items };
    console.log(`✅ Loaded ${categories.length} categories and ${items.length} items from Firestore`);
    ...
    console.warn(`⚠️ Local has MORE data (${currentTotalCount} vs ${firestoreTotalCount}) - keeping local`);
    ...
    console.log('✅ Using Firestore data (same count or Firestore is source of truth)');
    this.data.inventory = { categories, items };
    this.saveToStorage();
} else if (currentTotalCount > 0) {
    // Firestore empty but local has data
    console.warn('⚠️⚠️ Firestore is empty but local has data - keeping local and syncing');
    await this.syncInventoryToFirestore();
} else {
    // Both empty - data loss!
    console.error('❌❌ CRITICAL: Both Firestore AND local are empty!');
    console.error('❌ ALL INVENTORY DATA HAS BEEN LOST!');
}
...
console.log('⚠️ Keeping existing local inventory due to error');
return false;
    }
},
...
console.log('✅ Inventory synced to Firestore');
return true;
    } catch (error) {
...
if (type === 'category') {
    ...
    console.log('✅ Inventory category deleted from Firestore');
} else {
    await db.collection('inventory').doc('items').collection('list').doc(itemId).delete();
    console.log('✅ Inventory item deleted from Firestore');
}
return true;
    } catch (error) {
...
console.log(`✅ Loaded ${this.data.notifications.length} notifications from Firestore`);
return true;
    } catch (error) {
...
console.log('⚠️ All listeners already active');
return;
    }

    console.log('🔄 Setting up all real-time listeners...');
...
snapshot.docChanges().forEach((change) => {
    ...
    if (change.type === 'added' || change.type === 'modified') {
        ...
        if (index === -1) {
            this.data.team.push(teamMember);
            console.log('👷 Team member added/updated:', teamMember.name);
        } else {
        ...
        if (index !== -1) {
            this.data.team.splice(index, 1);
            console.log('👷 Team member removed:', teamMember.name);
            if (this.currentTab === 'team') {
                this.renderTeamTable();
            }
        }
...
.onSnapshot((snapshot) => {
    if (categoriesInitialLoad) {
        console.log('📦 Inventory categories listener - initial snapshot (skipping save)');
    ...
    if (removedCount > 3 && removedCount > this.data.inventory.categories.length * 0.5) {
        console.error(`⚠️⚠️ BLOCKED: Attempted to remove ${removedCount} categories (${this.data.inventory.categories.length} total). This looks like data loss!`);
    ...
    changes.forEach((change) => {
        ...
        if (change.type === 'added' || change.type === 'modified') {
            ...
            if (index === -1) {
                ...
                console.log(`📁 Category ${change.type}:`, category.name);
            } else {
    ...
    console.log(`📦 Total categories after update: ${this.data.inventory.categories.length}`);
...
.onSnapshot((snapshot) => {
    if (itemsInitialLoad) {
        console.log('📦 Inventory items listener - initial snapshot (skipping save)');
    ...
    if (removedCount > 5 && removedCount > this.data.inventory.items.length * 0.5) {
        console.error(`⚠️⚠️ BLOCKED: Attempted to remove ${removedCount} items (${this.data.inventory.items.length} total). This looks like data loss!`);
    ...
    changes.forEach((change) => {
        ...
        if (change.type === 'added' || change.type === 'modified') {
            ...
            if (index === -1) {
                ...
                console.log(`📦 Item ${change.type}:`, item.name);
            } else {
            ...
            if (index !== -1) {
                ...
                console.log(`🗑️ Item removed:`, item.name);
            }
        }
    });

    console.log(`📦 Total items after update: ${this.data.inventory.items.length}`);
    if (this.currentTab === 'inventory') {
        this.updateInventoryDisplay();
    }
    this.saveToStorage();
...
.onSnapshot((doc) => {
    if (doc.exists) {
        ...
        console.log('⚙️ Settings updated from Firestore');
        this.saveToStorage();
    }
});
...
console.log('⚠️ Listener already active - preventing duplicate!');
return;
    }
...
OMS.ordersUnsubscribe();
    ...
    console.log('🔄 Setting up real-time listener (SINGLE INSTANCE)');
...
if (isInitialLoad) {
    console.log(`📋 Initial snapshot received - loading ${snapshot.size} existing orders from Firebase`);
    isInitialLoad = false;
}
...
snapshot.docChanges().forEach((change) => {
    ...
    if (change.type === 'added') {
        ...
        if (!exists) {
            ...
            if (wasDeleted) {
                console.warn('⚠️ Order was previously deleted - ignoring:', docId);
                return;
            }

            if (!isFirstLoad) {
                console.log('🆕 New order detected:', orderId || `[docId: ${docId}]`);
            }
            ...
            if (!isFirstLoad) {
                this.updateAllDisplays();
                this.showToast(`🆕 New order: ${newOrder.clientName}`, 'success');
            }
        }
    }
...
if (change.type === 'modified') {
    const docId = change.doc.id;
    const orderId = orderData.orderId || '';
    console.log('✏️ Order modified:', orderId || `[docId: ${docId}]`);
...
this.saveToStorage(); // Save to local storage
this.updateAllDisplays();
this.showToast(`✏️ Order updated: ${this.data.orders[index].clientName}`, 'success');
    } else {
// FIXED: Check deletedOrders by docId AND orderId
const wasDeleted = this.data.deletedOrders.some(o =>
    o.docId === docId || (orderId && o.orderId === orderId)
...
if (wasDeleted) {
    console.warn('⚠️ Modified order was previously deleted - ignoring:', docId);
    return;
}

console.warn('⚠️ Modified order not found locally, adding it');
...
this.updateAllDisplays();
    ...
    if (change.type === 'removed') {
        console.log('🗑️ Order removed from Firestore:', orderData.orderId || change.doc.id);
        const index = this.data.orders.findIndex(o => o.orderId === (orderData.orderId || change.doc.id));
        if (index !== -1) {
            const deleted = this.data.orders.splice(index, 1)[0];
            this.updateAllDisplays();
            this.showToast(`🗑️ Order deleted: ${deleted.orderId}`, 'success');
        }
    }
});
...
if (isFirstLoad) {
    this.updateAllDisplays();
    console.log(`✅ Initial load complete - ${this.data.orders.length} orders loaded from Firebase`);
}
    });

    console.log('✅ Real-time listener active (SINGLE INSTANCE CONFIRMED)');
},

loadDemoData() {
    // DISABLED: Don't load demo data if we have real data in Firestore or deletedOrders
    console.log('📋 Checking if demo data should be loaded...');
    ...
    if (hasDeletedOrders) {
console.log('⚠️ Found deleted orders - skipping demo data (system has been used before)');
return;
    }

    if (hasRealInventory) {
console.log('⚠️ Found real inventory - skipping demo data');
return;
    }

    if (this.data.orders.length === 0) {
console.log('📋 No orders found. Loading demo data for first-time users...');
...
this.saveToStorage();
console.log('✅ Demo data loaded for first-time user');
    } else {
console.log('✅ Orders exist - demo data not needed');
    }
},
...
const notifications = pendingOrders.map(order => {
    ...
    if (daysUntil < 0) {
        urgency = `⚠️ OVERDUE by ${Math.abs(daysUntil)} days!`;
        color = 'var(--danger)';
    } else if (daysUntil === 0) {
        urgency = '🔥 TODAY';
        color = 'var(--danger)';
...
try {
    ...
    console.log('✅ Generated order ID from Firestore:', newOrderId);
    return newOrderId;
...
window.AdminUtils = {
    ...
    clearDeletedOrdersList() {
        ...
        OMS.saveToStorage();
        console.log(`✅ Cleared ${count} entries from deletedOrders list`);
        console.log('🔄 Refreshing from Firestore...');
        OMS.loadOrdersFromFirestore().then(() => {
            OMS.updateAllDisplays();
            console.log('✅ Sync complete! All Firestore orders should now be visible.');
        });
    },

    // Force full sync from Firestore (ignore localStorage)
    async forceFullSync() {
        console.log('🔄 Force syncing from Firestore...');
        OMS.data.deletedOrders = [];
    ...
    async deleteOrderFromFirestore(orderId) {
        try {
            ...
            if (docSnap.exists) {
                await docRef.delete();
                console.log(`✅ Deleted order ${orderId} from Firestore`);
                ...
                console.log(`✅ Added ${orderId} to deletedOrders list to prevent reappearing`);
                return true;
            } else {
    ...
    async findAndDeleteDemoOrders() {
        try {
            ...
            if (demoOrders.length === 0) {
                console.log('✅ No demo orders found!');
                return [];
            }

            console.log(`⚠️ Found ${demoOrders.length} potential demo orders:`);
            ...
            if (confirmed) {
                ...
                console.log(`✅ Deleted ${demoOrders.length} demo orders`);
            }

            return demoOrders;
        } catch (error) {
            console.error('❌ Error finding demo orders:', error);
        }
    },
    ...
    clearDeletedOrdersList() {
        OMS.data.deletedOrders = [];
        OMS.saveToStorage();
        console.log('✅ Cleared deletedOrders list');
    },
    ...
    viewDeletedOrders() {
        ...
        console.log(`📋 Total deleted orders: ${OMS.data.deletedOrders.length}`);
    },

    // Recover inventory FROM Firestore (force reload)
    async recoverInventoryFromFirestore() {
        try {
            console.log('🔄 Force loading inventory from Firestore...');
            ...
            if (categories.length === 0 && items.length === 0) {
                console.error('❌ Firestore inventory is empty! Nothing to recover.');
                console.log('💡 Use AdminUtils.recoverInventoryFromLocalStorage() if you have a localStorage backup');
                return false;
            }
            ...
            if (confirmed) {
                ...
                console.log('✅ Inventory recovered from Firestore!');
                console.log(`  Categories: ${categories.length}`);
                console.log(`  Items: ${items.length}`);
                return true;
            }
            ...
            console.error('❌ Error recovering inventory from Firestore:', error);
            return false;
        }
    },
    ...
    recoverInventoryFromLocalStorage() {
        try {
            ...
            if (!parsed.inventory || (!parsed.inventory.categories && !parsed.inventory.items)) {
                console.error('❌ No inventory data in localStorage backup');
                return false;
            }

            console.log('📦 Found inventory in localStorage backup:');
            ...
            if (confirmed) {
                ...
                OMS.saveToStorage();
                OMS.updateInventoryDisplay();
                console.log('✅ Inventory restored from localStorage backup!');
            ...
            console.error('❌ Error recovering inventory:', error);
            return false;
        }
    },
    ...
    async checkFirestoreInventory() {
        try {
            ...
            // Check categories
            const categoriesSnapshot = await db.collection('inventory').doc('categories').collection('items').get();
            console.log(`📁 Categories in Firestore: ${categoriesSnapshot.size}`);
            const categories = [];
            categoriesSnapshot.forEach(doc => {
            ...
            // Check items
            const itemsSnapshot = await db.collection('inventory').doc('items').collection('list').get();
            console.log(`📦 Items in Firestore: ${itemsSnapshot.size}`);
            const items = [];
            itemsSnapshot.forEach(doc => {
                const data = doc.data();
            ...
            console.error('❌ Error checking Firestore inventory:', error);
            return null;
        }
    },
    ...
    async deleteOrderByDocId(docId) {
        try {
            ...
            if (docSnap.exists) {
                ...
                await docRef.delete();
                console.log(`✅ Deleted order from Firestore - DocID: ${docId}, OrderID: ${data.orderId}`);
                ...
                OMS.saveToStorage();
                console.log(`✅ Added to deletedOrders list`);
                return true;
            } else {
                console.warn(`⚠️ Document ${docId} not found in Firestore`);
                return false;
            }
        } catch (error) {
            console.error('❌ Error deleting order:', error);
    ...
    async findGhostOrders() {
        try {
            console.log('👻 Searching for ghost orders (in local but not in Firestore)...');
            ...
            if (ghostOrders.length === 0) {
                console.log('✅ No ghost orders found! All local orders exist in Firestore.');
                return [];
            }
            ...
            console.log('\n💡 These orders exist locally but NOT in Firestore.');
            ...
            console.error('❌ Error finding ghost orders:', error);
            return [];
        }
    },
    ...
    removeGhostOrders() {
        ...
        console.log('⚠️ This will remove ALL orders that are NOT in Firestore.');
        ...
        OMS.loadOrdersFromFirestore().then(() => {
            OMS.updateAllDisplays();
            OMS.saveToStorage();
            console.log('✅ Ghost orders removed. Data reloaded from Firestore.');
        });
    },
    ...
    async permanentlyDeleteOrder(orderId) {
        ...
        if (!order) {
            console.warn(`⚠️ Order ${orderId} not found in local data`);
        }

        // STEP 1: Delete from Firestore (search by orderId field AND doc ID)
        try {
            console.log('🔍 Searching Firestore for this order...');
            ...
            if (!snapshot.empty) {
                ...
                await batch.commit();
                console.log('✅ Deleted from Firestore (by orderId field)');
            } else {
                ...
                if (docSnap.exists) {
                    await docRef.delete();
                    console.log('✅ Deleted from Firestore (by doc ID)');
                } else {
                    console.log('⚠️ Order not found in Firestore (may already be deleted)');
                }
            }
        } catch (error) {
        ...
        if (!existingInDeleted) {
            ...
            console.log('✅ Added to deletedOrders block list');
        } else {
            console.log('ℹ️ Already in deletedOrders list');
        }
        ...
        console.log(`\n✅ Order ${orderId} PERMANENTLY DELETED from:`);
        console.log('  ✓ Firestore database');
        console.log('  ✓ Local data');
        console.log('  ✓ Added to block list (deletedOrders)');
        console.log('  ✓ Saved to localStorage');
        console.log('\n🛡️ This order will NEVER reappear even after refresh!');
    },
    ...
    async checkDataConsistency() {
        ...
        if (firestoreOrderCount !== localOrderCount) {
            console.warn(`⚠️ Mismatch detected! ${Math.abs(firestoreOrderCount - localOrderCount)} orders difference`);
        ...
        if (categoriesSnapshot.size === 0 && itemsSnapshot.size === 0 &&
            ...
            console.error('❌ CRITICAL: Inventory exists locally but NOT in Firestore!');
    ...
    cleanupDeletedOrdersList() {
        console.log('🧹 Cleaning up deleted orders list...');
        const before = OMS.data.deletedOrders.length;
        const undefinedCount = OMS.data.deletedOrders.filter(o => !o.orderId).length;

        if (undefinedCount > 0) {
            console.log(`⚠️ Found ${undefinedCount} entries with undefined orderId`);
            OMS.data.deletedOrders = OMS.data.deletedOrders.filter(o => o.orderId && o.orderId !== 'undefined');
            OMS.saveToStorage();
            console.log(`✅ Removed ${before - OMS.data.deletedOrders.length} invalid entries`);
            console.log(`📋 Deleted orders list: ${before} → ${OMS.data.deletedOrders.length}`);
        } else {
            console.log('✅ No cleanup needed - all entries have valid orderIds');
        }
    },
    ...
    async debugEverything() {
        ...
        if (saved) {
            ...
            console.log(`  📦 Inventory categories: ${parsed.inventory?.categories?.length || 0}`);
            console.log(`  📦 Inventory items: ${parsed.inventory?.items?.length || 0}`);
            console.log(`  📋 Orders: ${parsed.orders?.length || 0}`);
            console.log(`  🗑️ Deleted orders: ${parsed.deletedOrders?.length || 0}`);
        ...
        console.log('\n═══════════════════════════════════════');
        console.log('2️⃣ CURRENT MEMORY (OMS.data)');
        console.log('═══════════════════════════════════════');
        console.log(`  📦 Inventory categories: ${OMS.data.inventory.categories.length}`);
        console.log(`  📦 Inventory items: ${OMS.data.inventory.items.length}`);
        console.log(`  📋 Orders: ${OMS.data.orders.length}`);
        console.log(`  🗑️ Deleted orders: ${OMS.data.deletedOrders.length}`);

        // 3. Check Firestore
        console.log('\n═══════════════════════════════════════');
        console.log('3️⃣ FIRESTORE CHECK');
        console.log('═══════════════════════════════════════');

        // Orders
        const ordersSnapshot = await db.collection('orders').get();
        console.log(`  📋 Orders in Firestore: ${ordersSnapshot.size}`);
        ...
        // 4. Find mismatches
        console.log('\n═══════════════════════════════════════');
        console.log('4️⃣ MISMATCH ANALYSIS');
        console.log('═══════════════════════════════════════');
        ...
        if (OMS.data.deletedOrders.length > 0) {
            ...
            for (const delOrder of OMS.data.deletedOrders) {
                ...
                if (!delOrder.orderId) {
                    console.warn(`  ⚠️ Skipping deleted order with undefined ID`);
                    continue;
                }
                ...
                if (!snapshot.empty) {
                    console.error(`  ❌ DELETED ORDER STILL IN FIRESTORE: ${delOrder.orderId}`);
                    snapshot.forEach(doc => {
                        console.error(`     Firestore Doc ID: ${doc.id}`);
                    });
                } else {
                    console.log(`  ✅ ${delOrder.orderId} - properly deleted from Firestore`);
                }
            }
        }
        ...
        if (ghostOrders.length > 0) {
            console.warn(`  ⚠️ Found ${ghostOrders.length} GHOST ORDERS (in local but NOT in Firestore):`);
            console.table(ghostOrders.map(o => ({
                OrderID: o.orderId,
                Client: o.clientName
            })));
        ...
        if (categoriesSnapshot.size === 0 && itemsSnapshot.size === 0) {
            console.error('  ❌ INVENTORY EMPTY IN FIRESTORE!');
            if (OMS.data.inventory.categories.length > 0 || OMS.data.inventory.items.length > 0) {
                console.log('  💡 Run: await OMS.syncInventoryToFirestore()');
            }
        } else if (OMS.data.inventory.categories.length === 0 && OMS.data.inventory.items.length === 0) {
            console.error('  ❌ INVENTORY EMPTY IN MEMORY!');
            console.log('  💡 Run: await AdminUtils.recoverInventoryFromFirestore()');
        }

        if (ghostOrders.length > 0) {
            console.log('  💡 Run: AdminUtils.removeGhostOrders() to clean them up');
        }
...
console.log('🛠️ Admin utilities loaded. Available commands:');
console.log('\n📋 ORDER COMMANDS:');
...
console.log('  - await AdminUtils.permanentlyDeleteOrder("ORDER_ID") - ⭐ PERMANENTLY DELETE (Firestore + local + block)');
...
console.log('  - await AdminUtils.recoverInventoryFromFirestore() - ⭐ LOAD FROM FIRESTORE');
...
console.log('  - await AdminUtils.debugEverything() - ⭐ FULL SYSTEM DIAGNOSTIC (find all bugs)');
...
// Validate dates
if (end < start) {
    container.innerHTML = '<div style="color: var(--danger); text-align: center; padding: 1rem;">⚠️ End date must be after start date!</div>';
    return;
}
...
if (diffDays > 30) {
    container.innerHTML = '<div style="color: var(--danger); text-align: center; padding: 1rem;">⚠️ Maximum 30 days allowed!</div>';
    return;
}
